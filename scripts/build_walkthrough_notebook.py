...
def build_cells() -> list[dict]:
    ...
    cells.append(md(
        ...
        "- `results/<config>/fingers.png` — the binarised final-frame view",
        "  with red tip markers and the critical-radius circle.",
        "- `results/<config>/cartesian.png` — the Cartesian alpha-field plot",
    ...
    cells.append(code(
        ...
        "        \"final_number_of_fingers\": m.get(\"final_number_of_fingers\"),",
        "        \"critical_radius_m\": m.get(\"critical_radius_m\"),",
        "    })",
    ...
    cells.append(md(
        ...
        "",
        "The next cell renders the Cartesian (X-Y) view of the alpha",
        "field at several time points and embeds it in this notebook:",
        "",
        ...
        "",
        "This cell runs **regardless of `RUN_SIMULATION`**: if `results/<config>/`",
        "exists on disk (e.g. from a previous run), the figures are rendered",
        "from it. If results are missing, the cell prints a clear message and",
        "skips that configuration. This lets you iterate on the visualization",
        "without re-running the heavy simulation.",
        "",
        "The next section ('The fingers -- number over time') shows the",
        "finger-count N(t) curve and the binarised final frame.",
    ))

    cells.append(code(
        "# Render the Cartesian fingers plot for each configuration",
        "# from the on-disk results. This runs regardless of RUN_SIMULATION:",
        "# if results/<config>/ is present, render the figures; otherwise",
        "# print a clear message and skip.",
        "from IPython.display import Image, display",
        "",
        "n_rendered = 0",
        "n_skipped = 0",
        "for cfg in CONFIGS:",
        "    cfg_results = Path(\"results\") / cfg",
        "    cfg_marker = cfg_results / \"system\" / \"controlDict\"",
        "    if not cfg_marker.exists():",
        "        print(f\"No results found for configuration {cfg}\")",
        "        print(f\"  (missing {cfg_marker})\")",
        "        print(\"  Set RUN_SIMULATION=1 and run the simulation cell\")",
        "        print(\"  to generate them, or pre-populate results/ with a previous run.\")",
        "        print()",
        "        n_skipped += 1",
        "        continue",
        "",
        "    out_cart = cfg_results / \"cartesian.png\"",
        "    try:",
        "        # Cartesian (X-Y) plot (the alien-like fingers). The",
        "        # polar view has been removed per the simplification: we",
        "        # only show the Cartesian scatter.",
        "        subprocess.run([",
        "            \"python\", \"openfoam/plot_fingers.py\",",
        "            \"--results-dir\", str(cfg_results),",
        ...
        "",
        "print()",
        "print(f\"Rendered {n_rendered} configuration(s), skipped {n_skipped}.\")",
    ))
    ...
    cells.append(code(
        ...
        "if RUN_SIM:",
        "    for cfg in CONFIGS:",
        "        alpha_path = Path(\"results\") / cfg / \"Timesteps\" / \"15\" / \"alpha.air\"",
    ...
    cells.append(md(
        "### The fingers — number over time, and the binarised final frame",
        "",
        "The finger pipeline reads the alpha field at every time-step dir,",
        "binarises it (\\alpha > 0.5), skeletonises it, and counts dead-end",
        "skeleton pixels (the 3x3-neighbour tip rule). The output is:",
        "",
        "- `n_fingers_over_time`: a dict mapping time-step string to finger count.",
        "- `final_number_of_fingers`: the count at the last time-step.",
        "- `critical_radius_m`: the smallest distance from the cell centre to",
        "  the air region's boundary, in metres.",
        "",
        "These are stored in `solution_metrics.json` alongside the five",
        "scalar metrics. The next cell plots the N(t) curve and embeds the",
        "binarised final-frame view (`fingers.png`).",
    ))

    cells.append(code(
        "# Plot the number of fingers over time and embed the binarised",
        "# final-frame view (fingers.png).",
        "import sys",
        "import matplotlib",
        "matplotlib.use(\"Agg\")",
        ...
        "",
        "if not RUN_SIM:",
        "    print(\"RUN_SIMULATION=0; skipping the finger pipeline plot.\")",
        "else:",
        "    for cfg in CONFIGS:",
        "        metrics_path = Path(\"results\") / cfg / \"solution_metrics.json\"",
        "        if not metrics_path.exists():",
        ...
        "            continue",
        "        m = json.loads(metrics_path.read_text())[\"metrics\"]",
        "        n_over_time = m.get(\"n_fingers_over_time\")",
        "        if not n_over_time:",
        "            print(f\"Configuration {cfg}: no n_fingers_over_time; skipping\")",
        "            continue",
        "        times = sorted(float(t) for t in n_over_time.keys())",
        "        counts = [n_over_time[f\"{t:g}\"] for t in times]",
        "        fig, ax = plt.subplots(figsize=(7, 4))",
        "        ax.plot(times, counts, \"-o\", color=\"steelblue\", markersize=3)",
        "        ax.set_xlabel(\"time (s)\")",
        "        ax.set_ylabel(\"number of fingers\")",
        "        ax.set_title(f\"N(t) — configuration {cfg} (alpha-field path)\")",
        "        ax.grid(True, alpha=0.3)",
        "        fig.tight_layout()",
        "        fig.savefig(f\"results/{cfg}/n_fingers_over_time.png\", dpi=125)",
        "        plt.close(fig)",
        "        print(f\"\\n--- Configuration {cfg} ---\")",
        "        print(f\"  final_number_of_fingers = {m.get('final_number_of_fingers')}\")",
        "        print(f\"  critical_radius_m       = {m.get('critical_radius_m'):.6f}\" if m.get('critical_radius_m') is not None else \"  critical_radius_m       = None\")",
        "        display(Image(filename=f\"results/{cfg}/n_fingers_over_time.png\", embed=True, width=700))",
        "        fingers_png = Path(f\"results/{cfg}/fingers.png\")",
        "        if fingers_png.exists():",
        "            display(Image(filename=str(fingers_png), embed=True, width=600))",
        "        else:",
        "            print(f\"  (no {fingers_png}; binarised frame not rendered)\")",
    ))
