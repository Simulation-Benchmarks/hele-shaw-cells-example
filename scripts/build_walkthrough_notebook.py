...
def build_cells() -> list[dict]:
    ...
    cells.append(code(
        ...
        "    os.chdir(REPO_DIR)",
        "else:",
        "    print(f\"{REPO_DIR} already exists; using the local clone.\")",
        "    # Fetch (but never reset -- hard) so we don't clobber uncommitted",
        "    # changes the user might have. The user is presumed to be on a",
        "    # branch and may have local edits we want to preserve.",
        "    subprocess.run([\"git\", \"-C\", REPO_DIR, \"fetch\", \"--depth\", \"1\", \"origin\", BRANCH], check=False)",
        "",
    ...
    cells.append(code(
        ...
        "    subprocess.run([\"docker\", \"build\", \"-t\", \"hele-shaw:latest\", \"openfoam/\"], check=True)",
